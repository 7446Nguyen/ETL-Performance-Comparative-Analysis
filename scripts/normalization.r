...
#OrderHeader
OrderHeader = oracle2 %>%
  select(po_no, site_num, request_date)

OrderHeader = OrderHeader %>%
  distinct(po_no,.keep_all = TRUE)

#order_line
order_line = oracle2 %>%
  select(`Line`,`Order`,ordered_item,`eaches_qty`,extended_price,po_no)

skimr::skim(order_line)
#item table
item = oracle2 %>%
